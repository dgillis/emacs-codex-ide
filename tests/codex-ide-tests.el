...
(ert-deftest codex-ide-start-from-menu-replaces-session-after-confirmation ()
  (let ((stopped nil)
        (started nil))
    (cl-letf (((symbol-function 'codex-ide--has-active-session-p)
               (lambda () t))
              ...
              ((symbol-function 'y-or-n-p)
               (lambda (prompt)
                 (should (string= prompt
                                  "Quit existing Codex session in /tmp/project and start a new one? "))
                 t))
...
(ert-deftest codex-ide-start-from-menu-keeps-session-when-not-confirmed ()
  (let ((stopped nil)
        (started nil))
    (cl-letf (((symbol-function 'codex-ide--has-active-session-p)
               (lambda () t))
              ...
              ((symbol-function 'codex-ide)
               (lambda ()
                 (setq started t)
                 'started)))
      (should-not (codex-ide--start-from-menu))
      (should-not stopped)
      (should-not started))))
