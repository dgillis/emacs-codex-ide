...
(ert-deftest codex-ide-start-replace-existing-keeps-single-empty-prompt ()
  (let ((project-dir (codex-ide-test--make-temp-project))
        (displayed nil)
        (requests '()))
    (codex-ide-test-with-fixture project-dir
        ...
        (cl-letf (((symbol-function 'codex-ide--display-buffer-in-side-window)
                  ...
                  ((symbol-function 'codex-ide--request-sync)
                   (lambda (_session method _params)
                     ...
                     (pcase method
                       ("thread/unsubscribe" '((ok . t)))
                       ("thread/start" '((thread . ((id . "thread-replaced")))))
                       (_ (ert-fail (format "Unexpected method %s" method)))))))
          (let ((session (codex-ide--create-process-session)))
            (setf (codex-ide-session-thread-id session) "thread-current")
            (with-current-buffer (codex-ide-session-buffer session)
              (insert "old transcript")
              (let ((result (codex-ide-start-replace-existing)))
                (should (eq result session))
                (should (eq displayed (codex-ide-session-buffer session)))
                (should (string= (codex-ide-session-thread-id session)
                                 "thread-replaced"))
                (should (equal (nreverse requests)
                               '("thread/unsubscribe" "thread/start")))
                (should (= (how-many "^> " (point-min) (point-max)) 1))
                (goto-char (point-max))
                (forward-line 0)
                (should (looking-at-p "> $"))))))))))
