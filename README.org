...
- Project-aware session startup, switching, stopping, and continue flows
...
Codex IDE reuses normal Emacs buffer display behavior when showing session
buffers. Set =codex-ide-focus-on-open= to control whether the selected window
switches to the displayed Codex buffer.
...
Codex IDE uses normal =display-buffer= behavior. To keep focus in the current
window when opening Codex:
